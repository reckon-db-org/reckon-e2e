...
Bumped all reckon-* dep floors to track the 2026-06 releases:

| Dep | Old | New |
|-----|-----|-----|
| `evoq` | `1.15.0` | `~> 1.21` |
| `reckon_evoq` | `2.1.1` | `~> 2.6` |
| `reckon_db` | `2.1.1` | `~> 5.2` |
| `reckon_gater` | `~> 2.3` | `~> 3.4` |

Key changes from the underlying libraries:
- **reckon-db 5.x**: breaking on-disk layout (Model C — `[streams,Type,Id,Ver]`).
  Existing local test stores must be re-created (the harness already does this per-run
  via `start_reckon_store/0`).
- **reckon-gater 3.4**: `{event_type, binary()}` added to `tag_filter()` union type;
  include paths renamed from `esdb_*` to `reckon_gater_*` (already correct since 0.4.0).
- **evoq 1.21**: `context_filter()` gains `{event_type, binary()}` leaf; compound
  filters union tag + event-type index reads before client-side refinement.
- **reckon-evoq 2.6**: requires `reckon_gater ~> 3.4`; no adapter API changes.

### Removed — CausationService proto + stubs

`CausationService` was removed from reckon-gateway (0.6.0) and reckon-proto (0.5.0).

- Deleted `priv/proto/reckon_causation.proto` from the vendored proto set.
- The generated `reckon_causation_pb.erl` and
  `reckon_gateway_v_1_causation_service_client.erl` stubs will no longer be
  regenerated by the grpcbox_plugin pre-compile hook (they were gitignored; no
  tracked files changed).

### Added — DCB proto vendored

- Added `priv/proto/reckon_dcb.proto` (reckon-proto 0.7.0). The grpcbox_plugin
  will generate `reckon_dcb_pb.erl` and
  `reckon_gateway_v_1_dcb_service_client.erl` stubs on first `rebar3 compile`.
  DCB RPC coverage test cases are not yet wired (see `gateway_rpc_coverage`).
